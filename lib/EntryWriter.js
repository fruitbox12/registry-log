...
class EntryWriter {
    ...
    update(entry, registered) {
        return new Promise(async (resolve, reject) => {
            if (registered) {
                ...
                entry = Object.assign({}, registered, { updated, sig: entry.sig, name: registered.name, target: entry.target });
                if (util_1.verify(entry)) {
                    this.feed.append(entry, async (err) => {
                        if (err)
                            return reject(err);
    ...
    remove(entry, registered) {
        return new Promise(async (resolve, reject) => {
            if (registered) {
                const updated = Date.now();
                entry = Object.assign({}, registered, { sig: entry.sig, updated, target: entry.target, removed: true });
                if (util_1.verify(entry)) {
                    this.feed.append(entry, async (err) => {
                        if (err)
                            return reject(err);
